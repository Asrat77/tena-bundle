...
module Cats
  module Core
    RSpec.shared_examples 'request_shared_spec' do |controller, field_count, exclude = []|
      ...
      let(:clazz) { "Comee::Core::#{controller.classify}".constantize }
      let!(:user) { create(:user) }
      let!(:headers) do
        application = create(:application_module, code: 'COMEE_CORE', name: 'Comee Core')
        role = create(:role, application_module: application)
        create(:user_application_role, user:, role:, application_module: application)
        token = Comee::Core::TokenService.issue(
          id: user.id, name: user.name, email: user.email, roles: user.application_roles(application).map(&:code)
        )
      ...
      unless exclude.include?(:index)
        describe 'GET /index' do
          it 'returns success response' do
            ...
            get(send("#{controller}_url"), headers:)
      ...
      unless exclude.include?(:show)
        describe 'GET /show' do
          it 'returns a success response' do
            obj = create(factory)
            get(send("#{controller.singularize}_url", obj), headers:)
            expect(response).to be_successful
            result = JSON(response.body)
            expect(result['success']).to be_truthy
